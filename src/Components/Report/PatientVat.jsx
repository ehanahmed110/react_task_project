...
export function PatientVat() {
    const [page, setPage] = useState(0);
    const [rows, setRows] = useState(10)
    const { vatReport, loading } = useSelector((state) => state.vatReport)
    const dispatch = useDispatch()
    useEffect(() => {
        dispatch(VatReportData({ start_date: "2025-05-30", end_date: "2025-06-30" }))
    }, [dispatch])
        ...
        <React.Fragment>
            <div>
                ...
                <div ref={componentRef}>
                    <div>
                        <h1 className='text-center text-xl font-medium capitalize text-black mb-4'>Patient VAT</h1>
                    </div>
                    <div>
                        <CoustomTable
                            //data={data}
