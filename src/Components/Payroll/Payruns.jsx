...
import { FetchPayrunsData } from "../../Features/Payroll/PayrunsSlice";

export function Payruns() {
    
 const [rows, setRows] = useState(10);
   const [page, setPage] = useState(0);
   const { payruns, loading } = useSelector((state) => state.payruns);
   ...
   return (
     <React.Fragment>
       <div className="flex justify-between">
         ...
         <div className="flex gap-x-3">
            <div>
           <ShareButton label="CREATE PAYROLL" icon="pi pi-plus" />
           </div>
           <div>
            <ShareButton label="GENERATE PAYRUNS" icon="pi pi-plus" />
           </div>
         </div>
       </div>
       <div>
